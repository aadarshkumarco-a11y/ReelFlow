...
export async function checkUploadStatus(containerId: string, accessToken: string): Promise<{ status_code: string; status: string }> {
  const res = await fetch(
    `${GRAPH_BASE}/${containerId}?fields=status_code&access_token=${accessToken}`
  )
  const data = await res.json()
  if (data.error) throw new Error(data.error.message)
  return { status_code: data.status_code, status: data.status_code === 'FINISHED' ? 'FINISHED' : 'IN_PROGRESS' }
}
